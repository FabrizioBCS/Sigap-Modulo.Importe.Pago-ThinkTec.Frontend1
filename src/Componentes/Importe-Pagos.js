      ...
      console.log(conceptoResto)
        ...
        .then((pagos)  =>{
         ...
         console.log(pagos);
          ...
          var arrayObligacion=[];
          var numa=0;
          fetch(CONFIG+'importealumnoobligaciones/search/'+this.state.codigoAlumno+'/'+this.state.codigoPrograma+'/1')
          .then((response)=>{
              return response.json()
          })
          .then((listaOblig)=>{
            console.log("Otras Obligaciones");
              ...
              /*if(listaOblig.length==1){
                ...
                listaOblig.forEach(function(data) {
                  numa=numa+1;
                    var e ={id: numa, id_tipo_oblig:data.id_tipo_obligacion,descripcion:data.descripcion,id_concepto:data.cod_concepto,  
          ...
          console.log(this.state.otrasObligaciones);
           ...
           /**OTRAS OBLIGACIONES PENDIENTES DE PAGO CANCELDAS */
           var arrayObligacionC=[];
           var numa=0;
           fetch(CONFIG+'importealumnoobligaciones/search/'+this.state.codigoAlumno+'/'+this.state.codigoPrograma+'/2')
           .then((response)=>{
               return response.json()
           })
           .then((listaOblig)=>{
             console.log("Otras Obligaciones Canceldasas");
               console.log(listaOblig);
           ...
           this.setState({
             otrasObligacionesC:arrayObligacionC,
           })
           console.log("Otras Obligaciones-Det-Canceldas");
           console.log(this.state.otrasObligacionesC);
        });
...
ActualizarObligacionesC=()=>{
  ...
  var valorTipoOb="";
    ...
    console.log("Tamaño Obligaciones");
      console.log(this.state.otrasObligacionesC.length);
      for(let j=0;j<this.state.otrasObligacionesC.length;j++){

            console.log("Tamaño TipoOblig");
            console.log(BD_TipoObligacion.length);
            for(let i=0;i<BD_TipoObligacion.length;i++){
              if(this.state.otrasObligacionesC[j].id_tipo_oblig==BD_TipoObligacion[i].id){
                valorTipoOb=BD_TipoObligacion[i].label;
                console.log("valorTipoOb");
                console.log(valorTipoOb);
              }
          ...
          console.log(this.state.datos.length);
          for(let i=0;i<this.state.datos.length;i++){
            if(this.state.otrasObligacionesC[j].id_concepto==this.state.datos[i].idConcepto){
              valorConcepto=this.state.datos[i].concepto;
              console.log("valorConcepto");
              console.log(valorConcepto);
            }
          ...
          console.log(this.state.monedas.length);
          for(let i=0;i<this.state.monedas.length;i++){
            if(this.state.otrasObligacionesC[j].id_moneda==this.state.monedas[i].id_moneda){
              valorMoneda=this.state.monedas[i].moneda;
              console.log("valorMoneda");
              console.log(valorMoneda);
            }
          ...
          console.log(BD_Estado.length);
          for(let i=0;i<BD_Estado.length;i++){
            if(this.state.otrasObligacionesC[j].id_estado==BD_Estado[i].id){
              valorEstado=BD_Estado[i].label;
              console.log("valorEstado");
              console.log(valorEstado);
            }
          ...
          var e ={id: this.state.otrasObligacionesC[j].id, 
                  val_tipo_oblig: valorTipoOb,
                  descripcion:this.state.otrasObligacionesC[j].descripcion,
                  val_concepto:valorConcepto,  
                  val_moneda:valorMoneda,
                  importe:this.state.otrasObligacionesC[j].importe,
                  val_estado:valorEstado};
                  ArrayActualizado.push(e);
      }
      console.log("ArrayActualizado");
